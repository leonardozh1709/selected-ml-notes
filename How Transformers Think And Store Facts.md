...
A standard Transformer [[Multi-Layer Perceptron (MLP)|MLP]] layer (ignoring bias terms for simplicity) takes an input vector $\mathbf{x}$ from the residual stream and performs two [[Linear Transformation|linear transformations]] with a non-linear activation $\sigma$ (like GELU):

$$\text{MLP}(\mathbf{x}) = \sigma(\mathbf{x} W_1) W_2$$

If we think of $W_1$ as matrix of key vectors $[k_1, ..., k_d]$, the [[Dot Product]] $\mathbf{x} \cdot \mathbf{k}_i$ (which happens during the [[Matrix-Matrix Products|Matmul]]) measures the unnormalized cosine similarity (directional alignment scaled by magnitude) of the current token state $\mathbf{x}$ against key vector $\mathbf{k}_i$: This product is bigger if the vectors **point in similar directions** and is also scaled by their magnitude.
...
- **Residual Addition:** The retrieved value $\mathbf{v}_i$ is written directly back into the token’s [[Residual Connections|residual stream]].
...
| ------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| Attention Key-Value | Dynamic representations derived from **other tokens in the prompt**. (Linear combinations) | **Context / Sequence** (Short-term memory) or additional signal ([[Attention#Cross-Attention\|Cross-Attention]]) |
...
The [Universal approximation theorem](https://en.wikipedia.org/wiki/Universal_approximation_theorem) applies to feedforward networks with a single hidden layer with non-polynomial activation functions. It states that [[Neural Networks — NEXUS|Neural Networks]] with a certain structure can theoretically approximate any continuous function to arbitrary accuracy.
